...
pre2014 <- medicaid_expansion %>%   #EG: what is this for? It's never used again. Delete from here?
  filter(year <= 2013)
...
ggplot(max_uninsured, aes(x = uninsured_rate, fill = State)) +  #EG: I changed `colour = State` to `fill`
  geom_bar()  # EG: a bar chart provides an awkward visualization (ie, y-axis), given that the y-axis is 
              ## the main point of a bar chart; could we use another visualization type?

#texas, nevada, florida, new mexico, georgia


##MJ Q: why filter (year==2013)? also should we use slice_min() as well?
```
...
ggplot(max_uninsured_pop, aes(x = uninsured_rate, colour = State)) +
  geom_bar()

# texas, florida, georgia, cali, ny

##MJ Q" why only year 2013? should we use "group_by(year) %>% summarize"?
```
...
ggplot(max_uninsured, aes(x = uninsured_rate, fill = State)) +  #EG: I changed `color = State` to `fill`
  geom_bar()  # EG: a bar chart provides an awkward visualization (ie, y-axis), given that the y-axis is 
              ## the main point of a bar chart; could we use another visualization type?
...
ggplot(max_uninsured_pop, aes(x = uninsured_rate, fill = State)) +
  geom_bar()
...
#new mexico, georgia
pre2014 %>%
filter(State %in% c("New Mexico", "Georgia")) %>%  # EG: Can you fix the indentation here? I did it above
ggplot() +
geom_point(aes(x = year,
y = uninsured_rate,
color = State)) +
